...
class EvalDispider(OVOBenchOffline):
    ...
    def _model_init(self):
        ...
        #         [StoppingCriteriaSub(stops=stop_words_ids)])
                ...
                # main text device
        self.main_device = self.model.get_input_embeddings().weight.device

        # try to find vision device
        try:
            self.vision_device = next(
                self.model.get_model().compressor.compressor.model.vision_tower.parameters()
            ).device
        except Exception:
            self.vision_device = self.main_device

        stop_words_ids = [
            torch.tensor(self.tokenizer('<|im_end|>').input_ids, device=self.main_device),
        ]
    ...
    def inference(self, video_file_name, prompt):
        ...
        input_ids = input_ids.unsqueeze(0).to(device=self.main_device, non_blocking=True)
        seqs = seqs.to(device=self.main_device, non_blocking=True)
        compress_mask = compress_mask.to(device=self.main_device, non_blocking=True)
        qs = qs.to(device=self.main_device, non_blocking=True)
        qs_mask = qs_mask.to(device=self.main_device, non_blocking=True)

        ans_token = self.time_tokenizer(
            DEFAULT_ANS_TOKEN, return_tensors="pt"
        ).input_ids.to(device=self.main_device, non_blocking=True)
        ...
        image_tensor = image_tensor.to(
            dtype=torch.float16, device=self.vision_device, non_blocking=True
        )
        image_tensor_large = image_tensor_large.to(
            dtype=torch.float16, device=self.vision_device, non_blocking=True
        )
        ...
        with torch.inference_mode():
            output_ids = self.model.generate(
                input_ids,
                images=image_tensor,
                images_large=image_tensor_large,
                seqs=seqs,
