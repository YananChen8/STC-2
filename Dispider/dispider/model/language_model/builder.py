...
def _resolve_local_reference(base_path, candidate):
...
class QwenCompressor(nn.Module):
    def __init__(self, compressor):
        ...
        vision_tower = getattr(self.config, 'mm_vision_tower', getattr(self.config, 'vision_tower', None))
        if vision_tower is None:
            raise ValueError(f'Unable to find `mm_vision_tower` in compressor config: {self.model_path}')
        vision_tower = _resolve_local_reference(self.model_path, vision_tower)
        self.vision_encoder = CLIPVisionTower(vision_tower, args=self.config)
...
def build_compressor(compressor_cfg):
    ...
    compressor = _resolve_local_reference(getattr(compressor_cfg, '_name_or_path', None), compressor)
