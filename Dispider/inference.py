...
class videoStream():
    def __init__(self, model_path, device="auto"):
        ...
        stop_words_ids = [
            torch.tensor(self.tokenizer('<|im_end|>').input_ids, device=self.device),
        ]
    ...
    def Run(self, file, prompt):
        ...
        input_ids = input_ids.unsqueeze(0).to(device=self.device, non_blocking=non_blocking)
        with torch.inference_mode():
            output_ids = self.model.generate(
                input_ids,
                images=image_tensor.to(dtype=self.dtype, device=self.device, non_blocking=non_blocking),
                images_large=image_tensor_large.to(dtype=self.dtype, device=self.device, non_blocking=non_blocking),
                seqs=seqs.to(device=self.device, non_blocking=non_blocking),
                compress_mask=compress_mask.to(device=self.device, non_blocking=non_blocking),
                qs=qs.to(device=self.device, non_blocking=non_blocking),
                qs_mask=qs_mask.to(device=self.device, non_blocking=non_blocking),
                ans_token=self.time_tokenizer(DEFAULT_ANS_TOKEN, return_tensors="pt").input_ids.to(device=self.device, non_blocking=non_blocking),
                todo_token=self.time_tokenizer(DEFAULT_TODO_TOKEN, return_tensors="pt").input_ids.to(device=self.device, non_blocking=non_blocking),
                q_id=None,
                insert_position=0,
                ans_position=[],
                do_sample=False,
...
def main():
    ...
    streamer = videoStream(args.model_path, device=args.device)
