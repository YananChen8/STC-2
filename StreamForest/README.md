...
The released environment is based on Python 3.10. If you use Python 3.11 or newer, `flash-attn` may fail to build. Install `flash-attn` only after `torch==2.1.2` is already present in the environment.
...
#### Running on a normal local GPU server

If your machine does not have `petrel_client` and stores videos on a normal local disk:

1. Copy `scripts/eval/local_data_roots.example.yaml` to a new file and replace the paths with your local dataset directories.
2. Export `STREAMFOREST_DATA_ROOTS_FILE` to point to that file.
3. If your machine is not using Slurm, set `STREAMFOREST_USE_SRUN=0`.

Example:

```bash
cd /path/to/StreamForest
source .venv/bin/activate
export PYTHONPATH=$PWD
export STREAMFOREST_DATA_ROOTS_FILE=$PWD/scripts/eval/local_data_roots.yaml
export STREAMFOREST_USE_SRUN=0
export NUM_GPUS=8

bash scripts/eval/run_eval.sh
```

You can also run a single benchmark directly:
