...
vision_process.VIDEO_MIN_PIXELS = int(os.environ.get('VIDEO_MIN_PIXELS', 100 * 28 * 28)) # follow qwen2vl paper
vision_process.FPS_MAX_FRAMES = int(os.environ.get('FPS_MAX_FRAMES', 480)) # decrease this for efficiency

FORCE_QWENVL_VIDEO_READER = vision_process.FORCE_QWENVL_VIDEO_READER
FPS_MAX_FRAMES = vision_process.FPS_MAX_FRAMES
VIDEO_MIN_PIXELS = vision_process.VIDEO_MIN_PIXELS
FRAME_FACTOR = vision_process.FRAME_FACTOR
FPS = vision_process.FPS
smart_nframes = vision_process.smart_nframes
smart_resize = vision_process.smart_resize

if hasattr(vision_process, 'IMAGE_FACTOR'):
    IMAGE_FACTOR = vision_process.IMAGE_FACTOR
else:
    IMAGE_FACTOR = 14 * getattr(vision_process, 'SPATIAL_MERGE_SIZE', 2)

if hasattr(vision_process, 'VIDEO_MAX_PIXELS'):
    VIDEO_MAX_PIXELS = vision_process.VIDEO_MAX_PIXELS
else:
    VIDEO_MAX_PIXELS = int(os.environ.get('VIDEO_MAX_PIXELS', 24576 * IMAGE_FACTOR * IMAGE_FACTOR))
    vision_process.VIDEO_MAX_PIXELS = VIDEO_MAX_PIXELS

if hasattr(vision_process, 'VIDEO_TOTAL_PIXELS'):
    VIDEO_TOTAL_PIXELS = vision_process.VIDEO_TOTAL_PIXELS
else:
    VIDEO_TOTAL_PIXELS = int(os.environ.get('VIDEO_TOTAL_PIXELS', VIDEO_MAX_PIXELS))
    vision_process.VIDEO_TOTAL_PIXELS = VIDEO_TOTAL_PIXELS
